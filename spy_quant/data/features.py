...
Feature set (FEATURE_DIM = 14):
  ...
  1  realized_vol        -- 20-bar rolling ? of log returns
  2  vol_of_vol          -- 20-bar rolling ? of realized_vol
...
_EPS = 1e-8
_TRADING_BARS_PER_DAY = 78   # 6.5 h x 12 bars/h
...
def compute_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # -- Drop warm-up rows -----------------------------------------------------
    out = out.dropna()
    logger.info(f"Features computed: {len(out):,} bars x {out.shape[1]} features")
    return out
